...
{% note warning %}
- **使用DOM0级方法只能在元素上注册一个事件处理程序。**
- **对于相同的事件，DOM0级事件处理程序与HTML事件处理程序事件处理程序无法共存。**原因自行思考。
- **使用DOM0级方法注册的事件处理程序被认为是元素的方法。因此，这时候的事件处理程序是在元素的作用域中运行，所以此时函数中的`this`指向元素自身。**
    ...
    <body>
        ...
        <script type="text/javascript">
            ...
            button.onclick = function (e) {
                ...
                console.log(event === arguments[0]) // true
...
- `listener`：作为事件处理程序的函数；
- `options（可选）`：一个对象。其属性如下：
    1. `capture`：一个布尔值，默认为`false`。当值为`true`时，表示要移除的`listener`是注册在事件捕获阶段的。
...
`event`对象包含与创建它的特定事件有关的属性和方法。触发的事件类型不一样，可用的属性和方法也不一样。下面简单的对事件对象的属性和方法进行了介绍，如果想查看详细信息，去[**MDN**](https://developer.mozilla.org/en-US/docs/Web/API/Event)看看吧。
...
{% note info %}
- `window.onload`：当整个页面，包括所有依赖资源（如样式表、图片等）都加载完成后，会触发该事件。你也可以通过在`<body>`元素上添加一个`onload`特性来为`window`注册`onload`事件处理程序。
- `image.onload`：当图像被加载完成后，会触发该事件。如果更改了图片的`src`属性，当新的图片加载完成后，会再次触发该事件。
- `script.onload`：当 JS 脚本被加载完成后，会触发该事件。如果更改`<script>`标签的`src`属性，当新的脚本加载完成后，会再次触发该事件。**需要注意的是，只有在设置了`src`属性并将`<script>`元素添加到文档后，才会开始加载 JS 脚本**。
- `link.onload`：当资源被加载完成后，会触发该事件。如果更改了`<link>`标签的`href`属性，当新的资源加载完成后，会再次触发该事件。**同样的，只有在设置了`href`属性并将`<link>`元素添加到文档后，才会开始加载相应的资源**。
- `xhr.onload`：当接收到完整的响应数据时，会触发该事件。
- `fileReader.onload`：当读取操作完成时，会触发该事件。
{% endnote %}

### readystatechange方法

{% note warning %}
**这是`document`对象和 xhr 对象上都有的一个方法，而不是事件。当`document`对象或 xhr 对象上的`readyState`属性变化时，就会自动调用该方法。**
{% endnote %}

{% note info %}
- `document.readyState`：
    - `loading`：正在加载文档。
    - `interactive`：文档已经加载并解析完成，但是一些像图片、样式表和子窗口的子资源还在加载中。
    - `complete`：所有资源都加载完成。这个状态暗示着，load 事件即将被触发。
- `xhr.readyState`：
    - `0`：未初始化。尚未调用`open()`方法。
...
{% note info %}
- 当初始的 HTML 文档被完全加载并解析成 DOM 树时，就会触发 DOMContentLoaded 事件，不必等待样式表、图像或自窗口的加载。
- 同步的`<script>`标签会阻塞 DOMContentLoaded 事件，但是异步的`<script>`标签则不会。
- 样式表的加载虽然不会阻塞 DOMContentLoaded 事件，但是由于样式表的加载和 DOM 解析是并行的，所以样式表的加载会使 DOM 的解析变慢。
- 该事件处理程序可以既可以注册在`window`上，也可以注册在`document`上。
- 必须使用 DOM2 级方法进行注册，才会生效。
{% endnote %}

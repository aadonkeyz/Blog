...
**`Reflect`对象是一个内置的对象，它是给底层操作提供默认行为的方法的集合。每个陷阱函数都可以在`Reflect`对象上找到与之对应的同名方法，这个方法也叫做反射接口，陷阱函数与反射接口之间是一一对应的，并且它们接收的参数是相同的。另外需要注意的是，`Reflect`不是一个函数对象，因此它不可以被当作构造器使用。**
...
3. **`handle.get()`的限制**：
    ...
    - 如果目标属性是不可配置的且没有定义`get`方法，那么返回值必须为`undefined`，否则会抛出错误。
{% endnote %}
...
console.log(proxy1.normalAttr)      // normal attribute
console.log(proxy1.banWrite)        // writable is false
console.log(proxy1.noGetter)        // undefined

console.log(proxy2.normalAttr)      // change the value
console.log(proxy2.banWrite)        // TypeError: 'get' on proxy: property 'banWrite' is a read-only and non-configurable data property on the proxy target but the proxy did not return its actual value (expected 'writable is false' but got 'change the value')
console.log(proxy2.noGetter)        // TypeError: 'get' on proxy: property 'noGetter' is a non-configurable accessor property on the proxy target and does not have a getter function, but the trap did not return 'undefined' (got 'change the value')
```

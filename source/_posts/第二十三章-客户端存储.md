...
- `domain`：Cookie对于哪个域是有效的。所有向该域发送的请求中都会包含这个Cookie信息。
    ...
    - 在高级域，可以通过将`domain`设置为`.`加当前域，来让所有的低级域也可以访问该Cookie。例如，在域为`wrox.com`时，设置`domain=.wrox.com`，则所有低级域也可访问该Cookie。**这条其实是老规范中定义的，新规范定义中浏览器会忽略前导`.`，默认低级域可以访问到比他高级的域设置的Cookie**。
...
- `max-age`：表示Cookie应该在多少秒后被删除。你别同时设置`expires`和`max-age`，因为我不太清楚具体哪个优先级高（网上说法好多，暂时没有进行验证）。

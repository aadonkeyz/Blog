...
function mirrorTree (root) {
  ...
  while (queue.length > 0) {
    ...
    if (preLevelNode) {
      const prevLeft = preLevelNode.left;
      const prevRight = preLevelNode.right;
      ...
      if (prevRight) {
        preLevelNode.right = {
          val: prevRight.val,
          left: prevRight.right,
          right: prevRight.left
        }
        queue.push(preLevelNode.right);
      }
    }
...
```js
/**
 * 这个递归有一个明显的缺陷，就是假设 root 下一定包含 p 和 q
 * 如果只包含 p 或 q 中的一个，算法就是不准确的
 */
...
function lowestCommonAncestor (root, p, q) {
  ...
  while (cur || stack.length > 0) {
    ...
    if (stack.length > 0) {
      ...
      if (lastVisited.right && lastHandled !== lastVisited.right) {
        cur = lastVisited.right;
      } else {
        lastHandled = stack.pop();
        
        lastVisited.contain = [lastVisited.val];
        if (lastVisited.left) {
          lastVisited.contain.push(...lastVisited.left.contain);
        }
        if (lastVisited.right) {
          lastVisited.contain.push(...lastVisited.right.contain);
        }

        if (lastVisited.contain.indexOf(p.val) !== -1 && lastVisited.contain.indexOf(q.val) !== -1 ) {
          return lastVisited;
        }
      }
    }

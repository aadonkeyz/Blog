...
从节点$n_1$到$n_k$的**路径**定义为节点$n_1, n_2, ···, n_k$的一个序列，使得对于$1 \leq i \leq k$的节点$n_i$是$n$<sub>$i+1$</sub>的父亲。这条路径的长是该路径上边的条数，即$k - 1$。从每一个节点到它自己有一条长为0的路径。注意，在一棵树中从根到每个节点恰好存在一条路径
...
{% note info %}
- **先序遍历**：第一个处理的节点为根节点。
- **中序遍历**：根节点在中间，左子树和右子树的中序遍历结果分别分布在根节点左右两侧。
- **后序遍历**：最后一个处理的节点为根节点。

由上述特点，可以得出结论：**先序+中序，或者中序+后序，可以得出二叉树的结构。**
{% endnote %}

```js
function reConstructTreeByPreMiddle(pre, middle) {
    let node = {
        value: pre[0],
        left: null,
        right: null
    ...
    let splitIndex = middle.indexOf(pre[0]);
    
    if (splitIndex > 0) {
        node.left = reConstructTreeByPreMiddle(pre.slice(1, splitIndex + 1), middle.slice(0, splitIndex));
    }

    if (splitIndex < middle.length - 1) {
        node.right = reConstructTreeByPreMiddle(pre.slice(splitIndex + 1), middle.slice(splitIndex + 1));
    }

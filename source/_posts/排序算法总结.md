...
首先将待排序数组转换为最大堆，因为数组中的最大元素总在 A[1] 中，通常把它与 A[heap.size] 进行互换，我们可以让该元素放到正确的位置。这时候，如果我们从堆中去掉最后一个结点（这一操作可以通过减少 heap.size 来实现），剩余的结点中，原来根的孩子结点仍然是最大堆，而新的根节点可能会违背最大堆的性质。为了维护最大堆的性质，我们要做的就是通过下滤操作使其符合最大堆的性质。堆排序算法会不断重复这一过程，直到 heap.size = 1。

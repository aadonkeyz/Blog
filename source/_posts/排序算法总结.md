...
{% note info %}
- **思路（从小到大排序）**：如果元素列是空或者只有一个元素，那么该元素列就是已排序完成的了（基本情况）。如果元素列里的元素超过一个，那么就从元素列中选取一个元素作为基准值，然后将元素列中小于基准值的元素放置在基准值左侧，大于基准值的元素放置在基准值右侧。然后重复递归的对基准值左侧和右侧两部分列执行该操作，直至最后。
- **时间复杂度（最坏）**：**O($n^2$)**
- **稳定性**：不稳定
{% endnote %}
